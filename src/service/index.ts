...
export default {
  ...
  ComDetails: async (number: any) => {
    return await service.get(`${baseUrl}api/Page/Details`, { params: { number } }).then(res => res).catch(err => err)
  },
  // 添加投诉
  ...
  // 审批操作
  AddApproval: async (data: any) => {
    return await service.post(`${baseUrl}api/T_Approval/Insert`, data).then(res => res).catch(err => err)
  },
  // 持续改进【科室】
  AddKaizen: async (data: any) => {
    return await service.post(`${baseUrl}api/T_Kaizen/Update`, data).then(res => res).catch(err => err)
  },
  // 科室调查
  AddInvestigation: async (data: any) => {
    return await service.post(`${baseUrl}api/T_Investigation/Insert`, data).then(res => res).catch(err => err)
  },
  // 获取字典下拉框信息 1 投诉人与患者的关系  3 投诉类型 4 审批操作 5 投诉方式 6 轻重程度 7 不良类型 8 不良发生地点 9 责任度
  ...
  // 获取责任人、当事人下拉框信息
  Party: async (id: number) => {
    return await service.get(`${baseUrl}api/T_User/Party`, { params: { id: id } }).then(res => res).catch(err => err)
  },
